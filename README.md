...
当前已发布版本为 `v0.1.3`，提供 Apple Silicon 版本，适用于 M1、M2、M3、M4 及后续 Apple 芯片 Mac：
...
| 自动更新 | 已接入（v0.1.4 起） | 设置页显示版本并检查更新；用户确认后下载、安装和重启 |
...
当前安装包尚未正式签名，升级后如果辅助功能开关已经开启但仍无法捕获，请完全退出 ReadFlow，在辅助功能列表中删除旧的 ReadFlow 条目，重新添加 `/Applications/ReadFlow.app`、打开开关并重启应用。
...
- 增加基于遗忘曲线的单词复习流程。
- 接入 PDF 文本提取。
- 增加 Intel / Universal 构建、Developer ID 签名和公证。
...
从 `v0.1.4` 起，应用会在“设置 → 版本与更新”中显示当前版本，并在发现新版本后由用户确认下载安装。发布者需要同时上传已签名的更新包和 `latest.json`；详见 [RELEASE.md](RELEASE.md)。
